...
export const permissaoTable = mysqlTable('permissao', {
    id: int('id').autoincrement().primaryKey(),
    nome: varchar('nome', { length: 50 }).notNull().unique(),
    createdAt: timestamp('createdAt').defaultNow()
});

export const permissaoInsertSchema = createInsertSchema(permissaoTable, {
    nome: z
...
export const permissaoRelations = relations(permissaoTable, ({ many }) => ({
